...
const authLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    console.log(
      `[RATE_LIMIT] Auth limit exceeded for IP: ${req.ip}, email: ${req.body?.email || 'unknown'}`
    );
    res.status(429).json(options.message);
  },
});
...
const strictLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    console.log(
      `[RATE_LIMIT] Strict limit exceeded for IP: ${req.ip}, path: ${req.path}`
    );
    res.status(429).json(options.message);
  },
});
...
const apiLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    console.log(
      `[RATE_LIMIT] API limit exceeded for user: ${req.user?.id || 'anonymous'}, IP: ${req.ip}`
    );
    res.status(429).json(options.message);
  },
});
