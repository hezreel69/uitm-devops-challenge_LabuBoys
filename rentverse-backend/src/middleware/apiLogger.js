...
const accessLogger = (req, res, next) => {
  ...
  res.end = function (chunk, encoding) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log(
        `[API] ${statusColor}${res.statusCode}\x1b[0m ${req.method} ${req.originalUrl} - ${duration}ms`
      );
    }
...
const securityLogger = {
  ...
  logRateLimitViolation(req, limiterType) {
    ...
    console.warn(
      `[SECURITY] Rate limit (${limiterType}) exceeded from ${req.ip}`
    );
    writeLog(SECURITY_LOG_FILE, formatLogEntry(entry));
  },
  ...
  logPasswordChange(req, userId) {
    ...
    console.log(
      `[SECURITY] Password changed for user ${userId} from ${req.ip}`
    );
    writeLog(SECURITY_LOG_FILE, formatLogEntry(entry));
  },
