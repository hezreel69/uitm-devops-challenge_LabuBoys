...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const result = await digitalAgreementService.getAgreementWithAccess(
      id,
      userId
    );
...
router.post('/:id/sign/landlord', auth, requireRecentAuth, async (req, res) => {
  try {
    ...
    const result = await digitalAgreementService.signAsLandlord(
      agreement.id,
      userId,
      {
        signature,
    ...
    if (
      ...
      error.message.includes('Only the landlord')
    ) {
      return res.status(403).json({
        success: false,
        error: error.message,
...
router.post('/:id/sign/tenant', auth, requireRecentAuth, async (req, res) => {
  try {
    ...
    const result = await digitalAgreementService.signAsTenant(
      agreement.id,
      userId,
      {
        signature,
    ...
    if (
      error.message.includes('Access denied') ||
      error.message.includes('Only the tenant') ||
      error.message.includes('Landlord must sign first')
    ) {
      return res.status(403).json({
        success: false,
        error: error.message,
      ...
      data: {
        ...
        signatures: {
          landlord: {
            ...
            signatureHash: agreement.landlordSignHash
              ? `${agreement.landlordSignHash.substring(0, 8)}...`
              : null,
          },
          tenant: {
            signed: agreement.tenantSigned,
            signedAt: agreement.tenantSignedAt,
            signatureHash: agreement.tenantSignHash
              ? `${agreement.tenantSignHash.substring(0, 8)}...`
              : null,
          },
        },
        ...
        verificationNote:
          'Document hashes can be verified against the original PDF to ensure integrity',
      },
    });
  } catch (error) {
...
router.post('/:id/cancel', auth, async (req, res) => {
  try {
    ...
    const result = await digitalAgreementService.cancelAgreement(
      agreement.id,
      userId,
      reason
    );
...
router.get('/:id/audit', auth, async (req, res) => {
  try {
    ...
    const auditLogs = await digitalAgreementService.getAuditTrail(
      agreement.id,
      userId
    );
...
router.get('/lease/:leaseId', auth, async (req, res) => {
  try {
    ...
    // Check access
    const hasAccess =
      agreement.lease.landlordId === userId ||
    ...
    const userRole =
      agreement.lease.landlordId === userId ? 'landlord' : 'tenant';
    ...
    res.json({
      ...
      data: {
        agreement,
        userRole,
        canSign: digitalAgreementService.canUserSign(
          agreement,
          userId,
          userRole
        ),
