...
function blacklistToken(token, expiresAt) {
  ...
  console.log(
    `[TOKEN_BLACKLIST] Token blacklisted. Total blacklisted: ${blacklistedTokens.size}`
  );
}
...
function blacklistUserTokens(userId, tokens) {
  ...
  console.log(
    `[TOKEN_BLACKLIST] Blacklisted ${tokens.length} tokens for user: ${userId}`
  );
}
...
function cleanupExpiredTokens() {
  ...
  if (removedCount > 0) {
    console.log(
      `[TOKEN_BLACKLIST] Cleaned up ${removedCount} expired tokens. Remaining: ${blacklistedTokens.size}`
    );
  }
}
