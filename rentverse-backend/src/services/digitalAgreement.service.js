...
class DigitalAgreementService {
  ...
  verifySignatureHash(storedHash, signature, timestamp, leaseId, userId) {
    const computedHash = this.createSignatureHash(
      signature,
      timestamp,
      leaseId,
      userId
  ...
  async initiateSigningWorkflow(agreementId, requestedBy, options = {}) {
    ...
    if (agreement.status !== 'DRAFT') {
      throw new Error(
        `Cannot initiate signing. Current status: ${agreement.status}`
      );
    }
  ...
  async signAsLandlord(agreementId, userId, signatureData) {
    ...
    const signHash = this.createSignatureHash(
      signature,
      timestamp,
      agreement.leaseId,
      userId
    ...
    try {
      ...
      if (leaseDetails) {
        ...
        const landlordName =
          ...
          leaseDetails.landlord.firstName ||
          'Landlord';
        const tenantName =
          leaseDetails.tenant.name || leaseDetails.tenant.firstName || 'Tenant';
      ...
      console.error(
        '❌ Error sending agreement completed emails:',
        emailError.message
      );
  ...
  async cancelAgreement(agreementId, userId, reason) {
    ...
    await this.logAuditAction(agreementId, 'CANCELLED', userId, null, {
      reason,
    });
