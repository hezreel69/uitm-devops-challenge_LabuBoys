...
async function importProperties() {
  try {
    ...
    const csvPath = path.join(
      __dirname,
      '../../../rentverse-datasets/rentals.csv'
    );
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      for (const row of batch) {
        try {
          ...
          const furnished =
            furnishedStr.includes('fully') || furnishedStr.includes('partial');
      ...
      console.log(
        `Progress: ${Math.min(i + batchSize, records.length)}/${records.length} (${importedCount} imported, ${skippedCount} skipped)`
      );
    }
